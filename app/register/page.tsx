...
export default function Page() {
    ...
    const handleRegister = async () => {
        if (mode) {
            if (usernameRef.current && usernameRef.current.value && passwordRef.current && passwordRef.current.value !== '' && passwordRef.current.value.length > 5 && confirmPasswordRef.current && confirmPasswordRef.current.value && confirmPasswordRef.current.value === passwordRef.current.value && emailRef.current && emailRef.current.value !== '') {
    ...
    return (
        <div className="w-full h-full p-8 flex items-center mt-[-160px]">
            <div className="w-[500px] h-fit p-6 mx-auto flex flex-col gap-2">
                ...
                {mode ? <Label>
                    Email
                    <Input className="mt-2" type="password" ref={emailRef} />
                </Label> : <></>}
                ...
                {mode ? <Label>
                    Nhập lại mật khẩu
                    <Input className="mt-2" type="password" ref={confirmPasswordRef} onKeyDown={e => { if (e.key == 'Enter') { handleRegister() } }} />
                </Label> : <></>}
